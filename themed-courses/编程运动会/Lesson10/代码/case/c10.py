...
def enemyAnim():
    global enemyFrm, enemyC,enemyNext,tigerFrm,tigerC,enemyEnd, enemyX, enemyY,atk, combo
    if enemyState == 'idle':
        if enemyTurn == 'left':
...
def playerHit():
    global enemyHP
    if plyTurn == 'right':
        rec1 = [x + 80, y + 65, x + 140, y + 70]
    elif plyTurn == 'left':
        rec1 = [x + 10, y + 60, x + 70, y + 70]
    rec2 = [enemyX + 85, enemyY + 85, enemyX + 115, enemyY + 105]
...
def enemyHit():
    global plyHP
    if enemyState == 'special':
        rec2 = [enemyX, enemyY + 50, enemyX + 150, enemyY + 80]
    elif enemyState == 'attack':
        if enemyTurn == 'right':
            rec2 = [enemyX + 70, enemyY + 55, enemyX + 190, enemyY + 70]
        elif enemyTurn == 'left':
            rec2 = [enemyX + 0, enemyY + 55, enemyX + 120, enemyY + 70]
    rec1 = [x + 40, y + 25, x + 80, y + 45]
    if collision(rec1, rec2):
...
def AI():
    global atk, combo, enemyTurn, enemyState, enemySpeed, enemyX, enemyY
    playerRootX = x + 30
    playerRootY = y + 85
    enemyRootX = enemyX + 60
    enemyRootY = enemyY + 120
    distance = ((playerRootX - enemyRootX) ** 2 + (playerRootY - enemyRootY) ** 2) ** 0.5
    if distance < 60 and playerRootY==enemyRootY:
        atk = True
        if combo < 3:
            enemyState = 'attack'
        else:
        ...
        if atk == False:
            enemyState = 'move'
            if playerRootX > enemyRootX:
                enemyX += enemySpeed
                enemyTurn = 'right'
            elif playerRootX < enemyRootX:
                enemyX -= enemySpeed
                enemyTurn = 'left'
            if playerRootY < enemyRootY:
                enemyY -= enemySpeed
            elif playerRootY > enemyRootY:
                enemyY += enemySpeed
...
while running:
    ...
    # 绘制结束界面
    if plyHP <= 0:
        screen.blit(bg1[1], (0, 0))
    elif enemyHP <= 0:
        screen.blit(bg1[0], (0, 0))
    # 全局更新及帧率
