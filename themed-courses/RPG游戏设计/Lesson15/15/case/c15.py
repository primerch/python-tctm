...
class Player(pygame.sprite.Sprite):
    ...
    # 生命值计算
    def calculateHP(self, damage=0):
        if damage <= self.Def:
            damage = self.Def + 1
        self.HP = self.HP - (damage - self.Def)
        if self.HP <= 0:
            self.HP = 0
            return True
        else:
...
class DamageNumber(pygame.sprite.Sprite):
    def __init__(self, number, pos):
        super().__init__()
        self.number = number
        self.image = pygame.image.load(f'images/UI/numbers/{number}.png')
        self.rect = self.image.get_rect()
        self.rect.topleft = pos
        self.speed = -2  # 伤害数字上升的速度
        self.start = pygame.time.get_ticks()  # 记录精灵创建的时间
        self.life = 1000  # 渐隐持续时间（毫秒）
    ...
    def update(self):
        ...
        if t >= self.life:
            ...
            # 计算当前透明度
            self.alpha = 255 - (255 * t / self.life)
            # 将透明度应用到图像上
            self.image.set_alpha(self.alpha)
            self.rect.y += self.speed

# 伤害数字精灵列表，需加入到精灵组
def showDamage(num, pos):
    pic = []
    sprite = DamageNumber('sign', (pos[0] + 5, pos[1] + 10))
    pic.append(sprite)
    for i, n in enumerate(num):
        sprite = DamageNumber(n, (pos[0] + (i + 1) * 22, pos[1]))
        pic.append(sprite)
    return pic
...
flag = False
def fight(sp, s):
    global count, gameRound, fightState, enemySkill, gameState, flag, timeWin, petName
    damage1, damage2 = 0, 0
        ...
        if count == 1:
            ...
            if sp == enemy:
                if enemySkill == 'attack':
                    damage1 = enemy.Atk
                elif enemySkill == 'skill1':
                    damage1 = enemy.skill1
                else:
                    damage1 = enemy.skill2
                if pet.calculateHP(damage1):
                    gameState = 'end'
                else:
                    gameRound = 'hero'
                    fightState = 'choose'
                    count = 0
                damageNum = str(damage1 - pet.Def)
                nums = showDamage(damageNum, (130, 300))
            else:
                if skill == 'attack':
                    damage2 = pet.Atk
                elif skill == 'skill1':
                    ...
                    damage2 = pet.skill2
                if enemy.calculateHP(damage2):
                    fight_sprites.add(win)
                    pet.changeAnim('win')
                    pet.Epx += 50
                    enemy.kill()
                    timeWin = pygame.time.get_ticks() # 获取刚播放胜利动画时的时间戳
                else:
                    gameRound = 'enemy'
                    enemySkill = random.choice(['attack', 'skill1', 'skill2'])
                    count = 0
                damageNum = str(damage2 - enemy.Def)
                nums = showDamage(damageNum, (800, 300))
            fight_sprites.add(nums)
        # 如果动画播放时长超过2秒且count大于等于3，返回地图界面
        elif pygame.time.get_ticks() - timeWin > 2000 and count >= 3:
            gameState = 'map'
            if pet.Epx == pet.MaxEpx:
                if petName[1] != '2':
                    petName = petName[0] + str(int(petName[1]) + 1)
            # 保存当前宠物的状态信息
            petData[petName]['HP'] = pet.HP
            petData[petName]['MP'] = pet.MP
            petData[petName]['Epx'] = pet.Epx
            # 删除战斗界面的pet对象
            pet.kill()
            # 删除精灵组中的敌人对象
            maps[enemyName].kill()
            ...
            del maps[enemyName]
            # 删除敌人字典中的敌人信息
            del mapEnemys[mapName][enemyName]
            # count计数重置为0
            count = 0
            # 将角色恢复至相应朝向的站立动画
            maps['hero'].changeAnim('stand' + direction)
    # 受击动画
    if flag:
...
def changeBar(s):
    ...
    if s.HP > s.MaxHP:
        s.HP = s.MaxHP
    ratioHP = s.HP / s.MaxHP
    ratioMP = s.MP / s.MaxMP
    if s == pet:
        petHP.image = pygame.transform.scale(petHP.image,(int(142*ratioHP),16))
        petMP.image = pygame.transform.scale(petMP.image,(int(142*ratioMP),16))
    elif s == enemy:
        enemyHP.image = pygame.transform.scale(enemyHP.image, (int(142 * ratioHP), 16))
        enemyMP.image = pygame.transform.scale(enemyMP.image, (int(142 * ratioMP), 16))

talk = False
charDisplayInterval = 100  # 控制字符显示速度（毫秒）

# 初始化时间
startTime = pygame.time.get_ticks()  # 每次调用前需要执行一次
text = "这是一个测试文本，用于展示逐字显示效果，并根据每行字数自动换行。"
lineLength = 25  # 每行字数
charSpacing = 5  # 字间距
lineSpacing = 10  # 行间距

def showText(text,name,pos=(0,0)):
    # 设置文本颜色
    color = (0, 0, 0)
    # 初始化行和列位置
    x, y = pos
    # 计算当前应该显示多少个字符
    currentTime = pygame.time.get_ticks()    # 当前时间
    numCharsToShow = (currentTime - startTime) // charDisplayInterval
    # 显示对话框背景
    img = pygame.image.load(f'images/UI/talk/{name}.png')
    screen.blit(img, (0, 0))
    # 显示字符
    for i in range(min(numCharsToShow, len(text))):
        char = text[i]
        textSurface = font.render(char, True, color)
        screen.blit(textSurface, (x, y))
        x += textSurface.get_width() + charSpacing
        # 如果达到行末尾，换行
        if (i + 1) % lineLength == 0:
            x = pos[0]
...
# 设置字体、字号 （Windows：simhei / MacOS：Arial）
...
while running:
        ...
        if e.type == pygame.QUIT:
            ...
            if gameState == 'map':
                ...
                # 确认功能
                    ...
                    if inputText.startswith("atk"):
                        try:
                            atk = int(inputText[3:])
                            petData[petName]['Atk'] = atk
                            print(petData[petName]['Atk'])
                        except ValueError:
                            print('无效指令')
                    elif inputText.startswith("hp"):
                        try:
                            hp = int(inputText[2:])
                            petData[petName]['HP'] = hp
                            petData[petName]['MaxHP'] = hp
                            print(petData[petName]['HP'],petData[petName]['MaxHP'] )
                        except ValueError:
                            print('无效指令')
                    inputText = ""  # 重置输入的文本
                # 输入功能
                else:
                    if e.key != pygame.K_ESCAPE and myCode == 1:
                        inputText += e.unicode
        # 松开按键，角色停止移动
        elif e.type == pygame.KEYUP:
            if gameState == 'map' and myCode == 0:
                if e.key == pygame.K_a:
            ...
            if e.button == 1:
                ...
                if gameState == 'hero':
                    ...
                    if click(c1, pos):
                        ...
                        # 调用进入地图函数
                        enterMap()
                if fightState == 'choose':  # 左键按下
                    # 检测鼠标点击的位置是否在精灵上
                    if click(btnAtk, pos):
                        skill, fightState = 'attack', 'done'
                        pet.calculateMP(-20)
                    elif click(btnSkill1, pos):
                        if pet.calculateMP(20):
                            skill, fightState = 'skill1', 'done'
                    elif click(btnSkill2, pos):
                        if pet.calculateMP(50):
                            skill, fightState = 'skill2', 'done'
    ...
    if gameState == 'start':
        ...
        lose = UI('images/UI/lose.png', pos=(0, 0))
        # 游戏失败精灵组
        loseSprites = pygame.sprite.Group(lose)
        # UI精灵组，控制基本UI元素
        ...
        if fightState == 'choose':
            ...
            # 显示HP、MP数值
            text = str(pet.HP) + '/' + str(pet.MaxHP)
            fs = font.render(text, True, 'white')
            screen.blit(fs, (210, 594))
            text = str(pet.MP) + '/' + str(pet.MaxMP)
            fs = font.render(text, True, 'white')
            screen.blit(fs, (210, 623))
            text = str(enemy.HP) + '/' + str(enemy.MaxHP)
            fs = font.render(text, True, 'white')
            screen.blit(fs, (750, 594))
            text = str(enemy.MP) + '/' + str(enemy.MaxMP)
            fs = font.render(text, True, 'white')
            screen.blit(fs, (750, 623))
        ...
        loseSprites.draw(screen)
    if talk:
        showText(text, 'a0',(320,550))
    if myCode == 1:
        # 调用函数，显示黑框及文本
        showCode(inputText, 50, 5)
pygame.quit()
sys.exit()