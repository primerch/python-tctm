    ...
    def handle(self, msg):
        if msg['cmd'] == 'login':
            ...
            if content == 'free':
                ...
                # 3. 设置消息框box的文本为msg['sender']向您发送视频聊天
                self.box.setText(msg['sender'] + "向您发送视频聊天")
                # 4. 给消息框box添加接听按钮和挂断按钮
                self.box.addButton("接听", QMessageBox.AcceptRole)
                self.box.addButton("挂断", QMessageBox.RejectRole)
                # 5. 显示消息框box，并获取其返回值result
                result = self.box.exec_()
                # 6. 如果result等于QMessageBox.AcceptRole，才创建视频窗口和设置videoFd属性
