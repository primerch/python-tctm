...
class AICam:
    ...
    def recognize(cls):
        ...
        """
        使用python的opencv库编写代码，已知程序中导入了opencv库并且获取了图像frame。
        现在你需要编写代码，满足如下要求：
            1. 创建一个名为"AICam"的窗口并设置为可调整大小的窗口模式。
            2. 根据frame图像的宽高比，将这个窗口的宽设定为200像素，高等比缩放。
            3. 将这个窗口的位置坐标设定为(0,0)。   
        请只输出满足要求的代码块，不要输出已知代码
        """
        cv2.namedWindow("AICam", cv2.WINDOW_NORMAL)
        aspect_ratio = frame.shape[1] / frame.shape[0]
        width = 200
        height = int(width / aspect_ratio)
        cv2.resizeWindow("AICam", width, height)
