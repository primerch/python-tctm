...
class ChatWindow(QWidget):
    ...
    def handle(self, msg):
        if msg['cmd'] == 'login':
            ...
            # 1. 如果退出好友name是聊天好友标签nameLb显示的好友名
            if self.nameLb.text() == name:
                # 2. 使用QMessageBox类创建消息框对象box，传入self
                box = QMessageBox(self)
                # 3. 设置box文本为name退出了, 并等待用户操作
                box.setText(name + '退出了')
                box.exec()
                # 4. 从消息列表字典中取出其对应的消息列表后关闭
                self.msgLws[name].close()
                # 5. 设置聊天好友标签文本为空
                self.nameLb.setText('')
                # 6. 隐藏消息输入框msgTxt和发送按钮sendBtn
                self.msgTxt.hide()
                self.sendBtn.hide()
