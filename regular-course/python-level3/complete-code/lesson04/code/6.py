...
class FlyObject:
    def __init__(self, img, x, y, speed=None):
...
fruits = []
fruits.append(Fruit('images/apple/full.png', 100, 600, speed=48))
fruits.append(Fruit('images/watermelon/full.png', 300, 600, speed=45))
for i in range(10):
    fruit = Fruit(random.choice(fruitImgs), random.randint(300, 900), 600)
    fruits.append(fruit)
...
bomb = Bomb('images/bomb/full.png', 500, 600, speed=48)
...
while True:
    ...
    bomb.move()
    knife.collide(bomb)
    # 1. 判断炸弹的中心点坐标centerY是否大于700
    if bomb.centerY > 700:
        # 2. 如果大于700，就将其替换为新创建的炸弹对象
        bomb = Bomb(
            'images/bomb/full.png',
