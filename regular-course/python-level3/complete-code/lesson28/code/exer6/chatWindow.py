...
class VideoWindow(QWidget):
    ...
    def closeEvent(self, event):
        # 2. 判断当前聊天客户端sender是否为视频发起者
        if self.sender == self.chatWindow.sender:
            # 3. 如果是，创建消息对象msg，传入'video','hangup',sender和to属性
            msg = Message('video', 'hangup', self.sender, self.to)
        # 4. 否则，则创建消息对象msg，传入'video','hangup',to和sender属性
        else:
            msg = Message('video', 'hangup', self.to, self.sender)
        # 5. 通过chatwindow对象client属性发送msg消息
        self.chatWindow.client.sendMsg(msg)
        # 6. 设置chatwindow对象的videoFd属性为空
