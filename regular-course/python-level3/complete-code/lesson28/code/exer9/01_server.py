    ...
    def handle(self, connSocket):
        while True:
            ...
            if req['cmd'] == 'login':
                ...
                # 5. 如果content为call,才调用video方法
                if req['content'] == 'call':
                    self.video(req)
                # 6. 如果content为hangup,则调用hangup方法
                elif req['content'] == 'hangup':
                    self.hangup(req)
        self.quit(connSocket)

    # 1. 定义hangup方法和req参数
    def hangup(self, req):
        # 2. 获取req的键sender和to的值，赋值给变量sender和to
        sender = req['sender']
        to = req['to']
        # 3. 将req字典转换为Message对象msg,并转发给to
        msg = Message(**req)
        self.sendMsg(self.connSockets[to], msg, 200)
        # 4. 从videoList中删除sender和to
        self.videoList.remove(sender)
        self.videoList.remove(to)

    def video(self, req):
        to = req['to']
        sender = req['sender']
        msg = Message(**req)
        if to not in self.videoList:
            self.sendMsg(self.connSockets[to], msg, 200)
            msg.content = 'free'
            self.sendMsg(self.connSockets[sender], msg, 200)
            self.videoList.append(to)
            self.videoList.append(sender)
        else:
            msg.content = 'busy'
            self.sendMsg(self.connSockets[sender], msg, 200)
