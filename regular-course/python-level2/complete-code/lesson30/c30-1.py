...
def greedy():
    global gold, g
    dict1 = {candyUI[key]['name']: key for key in candyUI}
    # print(dict1)
    b = buy
    g = 0
...
def drawMenu():
    ...
    # 卖出
        ...
        # 交易结果状态
            ...
            # 绘制收购明细
            x, i = 400, 0
            for name, num in soldData.items():
                if num > 0:
                    fs = font.render(f'{name}{num}个', True, 'brown')
                    screen.blit(fs, (x, 360 + i * 32))
                    i += 1
                    if i == 4:
                        x = 650
